...
int main(){
  ...
  while (cin >> v && v){
    ...
    for (int i=0; i<v; ++i) for (int j=i+1; j<v; ++j) assert(vowels[i] != vowels[j]);
    for (int i=0; i<c; ++i) for (int j=i+1; j<c; ++j) assert(consonants[i] != consonants[j]);
    ...
    ans.resize(words.size());
    copy(words.begin(), words.end(), ans.begin());
    ...
    if (ans.size() < 100) assert(ans.size() == count);
  }
  return 0;
}
