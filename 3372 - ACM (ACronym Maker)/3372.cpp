...
int main(){
  ...
  while (cin >> S && S){
    ...
    string acronym, acronymUpper, line;
    vector<string> w;
    getline(cin, line);
    while (getline(cin, line) && line.find("LAST CASE") != 0){
      ...
      /*
        dp[n][i][j] = La cantidad de maneras diferentes que puedo acomodar n letras
        del acrónimo scando al menos una de cada palabra y terminando en la letra
        j-ésima de la palabra i-ésima
       */
      ...
      if (t)
        cout << acronymUpper << " can be formed in " << dp[acronym.size()-1][w.size()-1][w.back().size()-1] << " ways" << endl;
      else
        cout << acronymUpper << " is not a valid abbreviation" << endl;
    }
